/*
  # Complete Database Setup for Vibe Finder
...
-- Drop existing types if they exist (to handle conflicts)
DROP TYPE IF EXISTS space_type CASCADE;
DROP TYPE IF EXISTS sound_type CASCADE;
DROP TYPE IF EXISTS signal_quality CASCADE;

-- Create custom types with correct values matching the application
...
-- Drop existing tables if they exist (to handle conflicts)
...
CREATE INDEX idx_spaces_updated_at ON spaces(updated_at DESC);
CREATE INDEX idx_samples_user_id ON samples(user_id);
CREATE INDEX idx_samples_space_id ON samples(space_id);
CREATE INDEX idx_samples_recorded_at ON samples(recorded_at DESC);
CREATE INDEX idx_audio_files_user_id ON audio_files(user_id);
CREATE INDEX idx_audio_files_sample_id ON audio_files(sample_id);
